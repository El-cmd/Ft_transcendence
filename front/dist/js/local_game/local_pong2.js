...
const leftPaddlePosition = new THREE.Vector3(0, 35, 90);
const rightPaddlePosition = new THREE.Vector3(0, 34, -90);
let animationID; //pour stocker l'ID de l'animation

export function initPongGame2Local() {
...
function handleResize() {
    ...
    if (!camera || !renderer) {
        console.log('Camera ou renderer non initialisé lors du redimensionnement');
        return;
    }
...
function setupPaddleMovement() {
    // Maj des mouvements uniquement pour le paddle local
        ...
        document.addEventListener("keydown", (event) => {
            if (event.key === "ArrowLeft") paddles.rightPaddle.userData.speed = -2;
            if (event.key === "ArrowRight") paddles.rightPaddle.userData.speed = 2;
        });
        document.addEventListener("keyup", (event) => {
...
function startBall(event) {
    if (event.code === "Space") {
        let directionZ = lastScorer === "player1" ? 2 : -2;
...
function updatePaddlePositions() {
    // Maj le paddle local uniquement
...
function displayWinnerMessage(winner) {
    ...
    setTimeout(() => {
        ...
        window.location.href = '/#/local_menu'; // Redirige vers le menu local
    }, 2000);
}
...
function resetGame() {
    ...
    scores.player2 = 0;
    updateScoreBoard();
    // Ici, vous pourrez également envoyer un message au back-end pour réinitialiser l'état global
}



function animate() {
    console.log('in animate, local pong 2');
    ...
    // Envoi de l'état de jeu au serveur (pour le joueur local)
    renderer.render(scene, camera);
}

export function cleanup2Local() {
